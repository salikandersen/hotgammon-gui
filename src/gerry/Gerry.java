...
public class Gerry {
  ...
  */
  ...
  public void play(GameAdapter game)
  {
	...
	int[] dicestate = game.getDiceState();
	Move move = play(boardstate, dicestate);
  ...
  */
  void makeMove(int [] board, int from, int to ) {
    ...
    // blot hitting test
    if ( board[to] == -1 ) {
      board[25]--; board[to] = 0; // move man to bar
    } 
    board[from]--; board[to]++;
  }
    
  boolean isRacing() { return isRacing(theboard); }
